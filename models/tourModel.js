    ...
    priceDiscount: {
      ...
      validate: {
        validator: function (val) {
          // this only points to the doc on new creation
          return this.price > val && val >= 0;
        },
        message:
          'A discount ({VALUE}) should be less than the price and not below zero  ',
      },
    },
    summary: {
      type: String,
...
//DOCUMENT  MIDDLEWARE : runs bef .save() and .create()
/*
tourSchema.pre('save', async function (next) {
console.log('guides', this.guides);
const guidesPromises = this.guides.map((id) => User.findById(id));
this.guides = await Promise.all(guidesPromises);
console.log('fin', this.guides);
next();
});
*/
...
/*
tourSchema.pre('save', function (next) {
  console.log('will save eventually XD');
  next();
});
